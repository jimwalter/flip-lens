// Local history store, persisted in chrome.storage.local.
//
// This is the default store and matches the original MVP behavior. Each entry
// holds a cropped thumbnail data URL, an editable title/description, an
// (editable) resale value, the auto-scraped confidence tier + reason, the
// Google Lens URL, and a timestamp.
//
// Data URLs can be large; chrome.storage.local has a generous quota which is
// sufficient for the MVP. If it ever proves too small, swap this module for an
// IndexedDB-backed one — the rest of the app only depends on this API.
